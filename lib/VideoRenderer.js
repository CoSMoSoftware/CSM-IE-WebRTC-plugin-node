...
function VideoRenderer(video) 
{
	...
	var self = this;
	this.onunload = window.addEventListener ("unload",function(){
		//Close nicely
		self.close();
	});
	...
	this.ontrackended = function() {
		//Set native object
		self.object.setTrack(null);
		//Delete from us
		delete (self.track);
	};
}

VideoRenderer.prototype.setTrack = function(track)
{
	//If we had a track
	if (this.track)
	...
	{
		//Set native object
		this.object.setTrack(null);
	} else {
		//Set native object
		this.object.setTrack(track.priv.track);
		//Add ended listener
