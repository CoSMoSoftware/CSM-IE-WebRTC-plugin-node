...
var RTCPeerConnection = function()
{
	...
	priv.cacheRtpSender = function(sender,track) 
	{
		...
		{
			//Create sender
			rtpSender = new RTCRtpSender(sender, track, this);
	...
	priv.cacheRtpReceiver = function(receiver, track)
	{
		...
		{
			//Create sender
			rtpReceiver = new RTCRtpReceiver(receiver, track, this);
	...
	priv.cacheRtpTransceiver = function(transceiver)
	{
		...
		if (!priv.transceivers.has(transceiver.mid)) 
		{
			...
			var rtpSender	= priv.cacheRtpReceiver(transceiver.sender);
	...
	priv.pc.onaddtrack = function (transceiver) {
		...
		//Get streams ids
		var streamIds = Utils.toArray(transceiver.receiver.getStreamIds());
		
		//Create new track object from native
		var track = new MediaStreamTrack(transceiver.receiver.track) 
		...
		var rtpReceiver		= priv.cacheRtpReceiver(transceiver.receiver, track);
		var rtpTransceiver	= priv.cacheRtpTransceiver(transceiver);
	...
	priv.pc.onremovestream = function (label) {
		//Delete from remote stream list
		delete(priv.remoteStreams[label]);
	};
};
...
RTCPeerConnection.prototype.addTransceiver = function(trackOrKind,init)
{
	...
	if (trackOrKind.priv && trackOrKind.priv.track) 
	{
		...
		var transceiver = this.pc.addTransceiverTrack(trackOrKind.priv.track, init);
		...
		var transceiver = this.pc.addTransceiverKind(trackOrKind, init);
		//Create transceiver
		rtpTransceiver = priv.cacheRtpTransceiver(transceiver);
	} else {
		//Error
 ...
 RTCPeerConnection.prototype.getTransceivers = function()
 {
	...
	if (!priv.pc || priv.isClosed)
	 ...
	 var transceivers = Utils.toArray(priv.pc.getTransceivers());
 ...
 RTCPeerConnection.prototype.getReceivers = function()
 {
	...
	if (!priv.pc || priv.isClosed)
	 ...
	 //Get the transceiver objects
	 var receivers = Utils.toArray(priv.pc.getReceivers());
 ...
 RTCPeerConnection.prototype.getSenders = function()
 {
	...
	if (!priv.pc || priv.isClosed)
	 ...
	 var senders = Utils.toArray(priv.pc.getSenders());
 ...
 RTCPeerConnection.prototype.getStats = function(selector)
{
	...
	return new Promise(function(resolve,reject) {
		...
		//For each one
		for (var i=0; i<senders.lenghth; ++i)
			//If it is the sender for this track
			if (senders[i].track === selector)
				//Get stats for it
				return priv.pc.getStats(callback,senders[i]);
		
		//Get receivers
		var receivers = self.getReceivers();
		
		//For each one
		for (var i=0; i<receivers.lenghth; ++i)
			//If it is the sender for this track
			if (receivers[i].track === selector)
				//Get stats for it
				return priv.pc.getStats(callback,receivers[i]);
